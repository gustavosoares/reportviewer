...
def map_dependencies_from_start_point(start):
	...
	#stack_open.append(start)
	while (len(stack_leafs) != 0):
		leaf = stack_leafs.pop()
		for father,childs in dependencies.items():
			if leaf in childs:
				print '>>> %s include %s' % (father, leaf)
				tree_nodes[father].add_children(tree_nodes[leaf])
		del dependencies[leaf]
	
	print '\nleafs removed\n'
	
	stack_open = dependencies[start_point]
	print 'stack inicial: %s' % stack_open
	previous_father = start_point
	while (len(stack_open) != 0):
		node = stack_open.pop()
		print 'analisando %s' % node

		if dependencies.has_key(node):
			childs = dependencies[node]
			print 'childs %s' % childs
			print 'stack open %s' % stack_open
			print 'previous_father %s' % previous_father
			print '************'
			for child in childs:
				tree_nodes[node].add_children(tree_nodes[child])
				stack_open.append(child)
			
			tree_nodes[previous_father].add_children(tree_nodes[node])
			previous_father = node
			del dependencies[node]
			print 'node %s removed' % node
			print '************'
		else:
			print '******'
	print '\n>>>>>> deps: %s\n' % dependencies
